# load word strength corpus, spell checker corpus, acronym dictionary, polarity corpus
import re, csv
...
politics={}
# load Politics corpus -> commonly occuring words used in politics
with open('corpus/PoliticsAddition.txt', 'rb') as f:
    all=f.readlines()
    for item in all:
        item=item.rstrip('\n').split()
        politics[item[0]]=float(item[1])/5.0
